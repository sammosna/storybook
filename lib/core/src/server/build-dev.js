...
const getReleaseNotesData = async (version) => {
  let result;
  try {
    await cache.remove('releaseNotesData');
    const fromCache = await cache.get('releaseNotesData', []);
    if (!fromCache.includes(version)) {
      await cache.set('releaseNotesData', [...fromCache, version]);
    }
    result = {
      success: true,
      isAutoMessagingEnabled: true,
      currentVersion: version,
      history: fromCache,
    };
  } catch (error) {
    result = { success: false, isAutoMessagingEnabled: true };
  }
  return result;
};
...
export async function buildDevStandalone(options) {
  try {
    ...
    const [port, versionCheck, releaseNotesData] = await Promise.all([
      ...
      releaseNotes
        ? getReleaseNotesData(version)
        : Promise.resolve({ success: false, isAutoMessagingEnabled: false }),
    ]);
